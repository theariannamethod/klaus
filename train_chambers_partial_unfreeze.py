...
BATCH=32; LR_CH=3e-4; LR_LM=1e-5; WARMUP=300; WD=0.1  # LR_LM=0: freeze LM weights
...
def load_emo():
    ...
    # EN GoEmotions
    p=f'{DATA_DIR}/en_goemotions_final.jsonl'
    if os.path.exists(p):
        with open(p) as f:
            for line in f:
                d=json.loads(line)
    ...
    # FR
    p=f'{DATA_DIR}/fr_goemotions_final.jsonl'
    if os.path.exists(p):
        with open(p) as f:
            for line in f:
    ...
    # RU GoEmotions
    p=f'{DATA_DIR}/ru_goemotions_final.jsonl'
    if os.path.exists(p):
        with open(p) as f:
            for line in f:
                d=json.loads(line); labs=d.get('labels',[])
                ...
                if 'chamber' in d:
                    ...
                    for li in labs:
                        ...
                        c=GO_TO_CH.get(ln,-1)
                        if c>=0: data['ru'].append((d['text'],c))
    for l in LANGS: print(f'  {l.upper()}: {len(data[l]):,} samples')
    return data
...
def train(steps):
    ...
    if os.path.exists(ckpt_path):
        ...
        print('  Checkpoint loaded (%.1f%%)' % (ckpt.get('acc',0)*100))
    # Partial unfreeze: only LM layers 3-5 (tunneling layers)
    lm_unfreeze=[]
    ch_params=[]
    frozen=0
    for name,p in model.named_parameters():
        if 'chambers' in name or 'res_proj' in name:
            ch_params.append(p)
        elif any(f'blocks.{i}.' in name for i in [3,4,5]) or 'ln.' in name or 'head.' in name:
            lm_unfreeze.append(p)
        else:
            p.requires_grad=False
            frozen+=p.numel()
    n_lm=sum(p.numel() for p in lm_unfreeze)
    n_ch=sum(p.numel() for p in ch_params)
    print(f'  LM layers 3-5 unfrozen: {n_lm:,} (lr={LR_LM})')
    print(f'  Chamber params: {n_ch:,} (lr={LR_CH})')
    print(f'  Frozen (layers 0-2 + emb): {frozen:,}')
    opt=torch.optim.AdamW([
        {'params':lm_unfreeze,'lr':LR_LM},
        {'params':ch_params,'lr':LR_CH},
    ...
    for step in range(1,steps+1):
        ...
        if step%2000==0 or step==steps:
            ...
            if ta>best_acc:
                ...
                # Save chambers + res_projs + ALL LM weights
                raw = model._orig_mod if hasattr(model, '_orig_mod') else model
                sd_lms = {l: raw.lms[l].state_dict() for l in LANGS}
