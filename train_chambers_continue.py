...
def train(steps):
    ...
    # Load existing chambers checkpoint (continue training, not from scratch)
    ckpt_path=f'{WEIGHTS_DIR}/klaus_chambers.pt'
    if os.path.exists(ckpt_path):
        ...
        print(f'  Chambers loaded from checkpoint (acc={ckpt.get("acc","?"):.1%})')
    # Freeze LM weights
    for l in LANGS:
    ...
    # Per-language class weights to fix LOVE dominance in RU
    ru_counts=[589,22827,4133,1417,3366,1202]
    ru_weights=torch.tensor([max(ru_counts)/max(1,c) for c in ru_counts],dtype=torch.float,device=dev)
    ru_weights=ru_weights/ru_weights.sum()*6
    crit=nn.CrossEntropyLoss()
    crit_ru=nn.CrossEntropyLoss(weight=ru_weights)
    print(f'{"─"*60}')
    ...
    for step in range(1,steps+1):
        ...
        loss=crit_ru(raw,tgt) if lang=='ru' else crit(raw,tgt)
