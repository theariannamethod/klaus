...
def load_emo():
    ...
    for gl in LANGS:
        ...
        if os.path.exists(gp):
            ...
            if n>0: print('  %s GPT: +%d samples'%(gl.upper(),n))
...
def train(steps):
    ...
    if os.path.exists(ckpt_path):
        ...
        print('  Continuing from %.1f%%' % (ckpt.get('acc',0)*100))
    # Freeze LM weights
    for l in LANGS:
    ...
    print(f'{"─"*60}')
    best_acc=ckpt.get('acc',0) if 'ckpt' in dir() else 0; t0=time.time(); losses=[]; accs=[]
