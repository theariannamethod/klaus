...
RES_DIM=100; N_CHAMBERS=6; CF_ITERS=5; CF_K=0.05
BATCH=32; LR_CH=3e-4; LR_LM=0; WARMUP=300; WD=0.1  # LR_LM=0: freeze LM weights
...
class Chambers(nn.Module):
    ...
    def forward(s,res):
        ...
        act=torch.sigmoid(raw)
        # FIX 2: decay outside loop (was decay^5, now decay^1)
        # FIX 3: linear coupling instead of sin() on [0,1]
        for _ in range(CF_ITERS):
            diff=act.unsqueeze(2)-act.unsqueeze(1) # (B,6,6) linear diff
            infl=CF_K*diff
            act=act+(infl*s.coupling.to(act.device).unsqueeze(0)).sum(2)
            act=act.clamp(0,1)
        act=act*s.decay.to(res.device) # decay once
        return act, raw
...
class KlausFull(nn.Module):
    ...
    def forward(s,tokens,lang):
        ...
        # FIX 1: Masked mean pooling (ignore padding tokens)
        if tokens is not None:
            mask = (tokens != 0).unsqueeze(-1).float()  # (B,S,1)
            pooled = (mid * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1)
        else:
...
def load_emo():
    ...
    # EN GoEmotions
    p=f'{DATA_DIR}/en_goemotions_v3.jsonl'
    if os.path.exists(p):
        with open(p) as f:
            for line in f:
                d=json.loads(line)
    ...
    # FR
    p=f'{DATA_DIR}/fr_goemotions_v3.jsonl'
    if os.path.exists(p):
        with open(p) as f:
            for line in f:
    ...
    # RU GoEmotions
    p=f'{DATA_DIR}/ru_goemotions_v3.jsonl'
    if os.path.exists(p):
        with open(p) as f:
            for line in f:
                d=json.loads(line); labs=d.get('labels',[])
    ...
    # GPT-generated emotion data (all langs)
    for gl in LANGS:
        gp=DATA_DIR+'/%s_gpt_emotion.jsonl'%gl
        if os.path.exists(gp):
            ...
            with open(gp) as f:
                for line in f:
                    d=json.loads(line)
                    if 'chamber' in d:
                        data[gl].append((d['text'],d['chamber'])); n+=1
            if n>0: print(f'  {gl.upper()} GPT: +{n} samples')
...
def split_val(emo, val_frac=0.15):
    """Proper validation split."""
    train_data = {}; val_data = {}
    for l in emo:
        random.shuffle(emo[l])
        n_val = int(len(emo[l]) * val_frac)
        val_data[l] = emo[l][:n_val]
        train_data[l] = emo[l][n_val:]
        print("  %s: train=%d, val=%d" % (l.upper(), len(train_data[l]), len(val_data[l])))
...
def train(steps):
    ...
    if os.path.exists(ckpt_path):
        ...
        model.chambers.load_state_dict(ckpt['chambers'], strict=False)
        for l in LANGS:
            if l in ckpt.get('res_projs',{}):
                model.res_projs[l].load_state_dict(ckpt['res_projs'][l])
        print(f'  Checkpoint loaded ({ckpt.get("acc",0):.1%})')
    # Freeze LM weights
    for l in LANGS:
    ...
    # FIX 12: Precompute by_ch per language (was rebuilt every step)
    precomp_by_ch = {}
    for l in active:
        by_ch_l = [[] for _ in range(N_CHAMBERS)]
        for t,c in emo[l]: by_ch_l[c].append((t,c))
        precomp_by_ch[l] = by_ch_l
    for step in range(1,steps+1):
        model.train()
        lang=random.choice(active)
        # Balanced sampling: equal samples per chamber
        by_ch=precomp_by_ch[lang]
        batch=[]
        ...
        if step%2000==0 or step==steps:
            model.eval(); print(f'\n  >>> EVAL step {step}')
            for el in active:
                samp=random.sample(val_emo[el],min(200,len(val_emo[el])))
