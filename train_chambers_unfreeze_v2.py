...
BATCH=32; LR_CH=3e-4; LR_LM=5e-6; WARMUP=300; WD=0.1  # LR_LM=0: freeze LM weights
...
def train(steps):
    ...
    for name,p in model.named_parameters():
        if 'chambers' in name or 'res_proj' in name:
            ...
            lm_params.append(p)
    n_lm=sum(p.numel() for p in lm_params)
    n_ch=sum(p.numel() for p in ch_params)
    print(f'  LM params: {n_lm:,} (lr={LR_LM})')
    print(f'  Chamber params: {n_ch:,} (lr={LR_CH})')
    opt=torch.optim.AdamW([
        {'params':lm_params,'lr':LR_LM},
